...
class Blockchain:
    ...
    """Keep Track of all the unspend Transaction in cache memory for fast retrival"""
    ...
    def addBlock(self, BlockHeight, prevBlockHash):
        ...
        merkleRoot = merkle_root(self.TxIds)[::-1].hex()  # Placeholder value, you need to compute the actual merkle root

        blockHeader = BlockHeader(VERSION, prevBlockHash, merkleRoot, timestamp, self.bits)
        blockHeader.mine(self.current_target)  # Attempt to mine the block header
        self.remove_spent_Transactions()
